"""subscriptions için iki-org RLS izolasyon testi (B6). test_compliance_rls.py deseni."""
import os
import uuid
...
def _count_subs_in(conn, org_ids) -> int:
    return conn.execute(
        text("SELECT count(*) FROM subscriptions WHERE org_id = ANY(:oids)"),
        {"oids": [str(o) for o in org_ids]},
    ).scalar()


def test_subscriptions_rls_isolates_orgs_and_bypass_sees_all():
    ...
    try:
        ...
        for oid, name in [(org_a, "RLS Sub A"), (org_b, "RLS Sub B")]:
            conn.execute(
        ...
        assert _count_subs_in(conn, [org_a, org_b]) == 1  # B gizli
        conn.execute(text("SELECT set_config('app.bypass_rls', 'on', true)"))
        assert _count_subs_in(conn, [org_a, org_b]) == 2
    finally:
        trans.rollback()
        conn.close()
        eng.dispose()
...
def test_subscriptions_fail_closed_without_guc():
    eng = create_engine(DB_URL, future=True)
    conn = eng.connect()
    trans = conn.begin()
    try:
        _assert_non_superuser(conn)
        assert conn.execute(text("SELECT count(*) FROM subscriptions")).scalar() == 0
    finally:
        trans.rollback()
        conn.close()
        eng.dispose()
