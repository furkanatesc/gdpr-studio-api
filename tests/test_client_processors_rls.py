"""client_processors iki-org RLS izolasyon testi (FORCE RLS, kvkk_app non-superuser)."""
import os
import uuid
...
def _count(conn, org_ids) -> int:
    return conn.execute(
        text("SELECT count(*) FROM client_processors WHERE org_id = ANY(:oids)"),
        {"oids": [str(o) for o in org_ids]},
    ).scalar()


def test_client_processors_rls_isolates_orgs_and_bypass_sees_all():
...
def test_client_processors_fail_closed_without_guc():
    eng = create_engine(DB_URL, future=True)
    conn = eng.connect()
    trans = conn.begin()
    try:
        _assert_non_superuser(conn)
        assert conn.execute(text("SELECT count(*) FROM client_processors")).scalar() == 0
    finally:
        trans.rollback()
        conn.close()
        eng.dispose()
